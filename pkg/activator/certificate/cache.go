...
import (
	...
	configmapinformer "knative.dev/pkg/injection/clients/namespacedkube/informers/core/v1/configmap"
...
func NewCertCache(ctx context.Context) *CertCache {
	...
	configmapInformer.Informer().AddEventHandler(cache.FilteringResourceEventHandler{
		FilterFunc: reconciler.ChainFilterFuncs(
			reconciler.LabelExistsFilterFunc(KnativeCABundleLabelKey),
		),
...
func (cr *CertCache) updateCertificate(secret *corev1.Secret) {
	...
	if err != nil {
		cr.logger.Warnf("failed to parse secret %s/%s: %v", system.Namespace(), netcfg.ServingRoutingCertName, zap.Error(err))
		return
	}
	cr.certificate = &cert
}
...
func (cr *CertCache) addTrustBundles(pool *x509.CertPool) {
	selector, err := getLabelSelector(KnativeCABundleLabelKey)
	if err != nil {
	...
	if err != nil {
		cr.logger.Warnf("Failed to get ConfigMaps %s/%s with label %s: %v", system.Namespace(),
			netcfg.ServingRoutingCertName, KnativeCABundleLabelKey, zap.Error(err))
		return
	}
