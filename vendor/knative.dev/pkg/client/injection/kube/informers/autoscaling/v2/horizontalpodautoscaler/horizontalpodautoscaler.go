...
func init() {
	fmt.Println("Running init on v2")
...
func withInformer(ctx context.Context) (context.Context, controller.Informer) {
	...
	return context.WithValue(ctx, injection.HPAV2Key{}, inf), inf.Informer()
}

func withDynamicInformer(ctx context.Context) context.Context {
	inf := &wrapper{client: client.Get(ctx), resourceVersion: injection.GetResourceVersion(ctx)}
	return context.WithValue(ctx, injection.HPAV2Key{}, inf)
}

// Get extracts the typed informer from the context.
func Get(ctx context.Context) v2.HorizontalPodAutoscalerInformer {
	untyped := ctx.Value(injection.HPAV2Key{})
	if untyped == nil {
